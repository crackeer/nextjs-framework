/**
 * JSON 工具函数集
 * 包含：superDecode（递归展开）、jsonToGo（转 Go 结构体）
 */

/**
 * 递归解码：将 JSON 字符串中嵌套的 JSON 字符串层层展开为对象
 * @param {*} value
 * @returns {*}
 */
export function superDecode(value) {
    ...
    if (dataType === 'object') {
        if (Array.isArray(value)) {
            ...
            for (let i = 0; i < keys.length; i++) {
                value[keys[i]] = superDecode(value[keys[i]]);
            }
        }
    }
    return value;
...
/**
 * 解码：双重 JSON.parse（处理被转义的 JSON 字符串）
 * @param {string} text
 * @returns {*}
 */
...
/**
 * JSON-to-Go 转换
 * 原始实现: https://github.com/mholt/json-to-go
 * @param {string} json     - JSON 字符串
 * @param {string} typename - 顶层类型名（默认 "AutoGenerated"）
 * @param {boolean} flatten - 是否展平（默认 true）
 * @param {boolean} example - 是否生成示例（默认 false）
 * @param {boolean} allOmitempty - 全部添加 omitempty（默认 false）
 * @returns {{ go: string, error?: string }}
 */
export function jsonToGo(json, typename, flatten = true, example = false, allOmitempty = false) {
    ...
    function indent(tabs) { for (let i = 0; i < tabs; i++) go += '\t'; }
    function append(str) { go += str; }
    function indenter(tabs) { for (let i = 0; i < tabs; i++) stack[stack.length - 1] += '\t'; }
    function appender(str) { stack[stack.length - 1] += str; }

    function uniqueTypeName(name, seen) {
        if (seen.indexOf(name) === -1) return name;
        let i = 0;
        while (true) {
            let newName = name + i.toString();
            if (seen.indexOf(newName) === -1) return newName;
            i++;
        }
    }
